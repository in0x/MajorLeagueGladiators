...
void UPackMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	QUICK_SCOPE_CYCLE_COUNTER(STAT_PackMovementComponent_TickComponent);
...
bool UPackMovementComponent::HandleDeflection(FHitResult& Hit, const FVector& OldVelocity, const uint32 NumBounces, float& SubTickTimeRemaining)
{
...
bool UPackMovementComponent::HandleSliding(FHitResult& Hit, float& SubTickTimeRemaining)
{
...
void UPackMovementComponent::SetVelocityInLocalSpace(FVector NewVelocity)
{
	if (UpdatedComponent)
	{
...
FVector UPackMovementComponent::ComputeVelocity(FVector InitialVelocity, float DeltaTime) const
{
	// v = v0 + a*t
...
FVector UPackMovementComponent::LimitVelocity(FVector NewVelocity) const
{
...
FVector UPackMovementComponent::ComputeMoveDelta(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UPackMovementComponent::ComputeAcceleration(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UPackMovementComponent::ComputeBounceResult(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
void UPackMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice, const FVector& MoveDelta)
{
...
bool UPackMovementComponent::CheckStillInWorld()
{
	if (!UpdatedComponent)
	{
		return false;
...
bool UPackMovementComponent::ShouldUseSubStepping() const
{
	return bForceSubStepping || (GetGravityZ() != 0.f);
}

float UPackMovementComponent::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const
{
