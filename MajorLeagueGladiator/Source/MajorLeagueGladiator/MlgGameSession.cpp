	...
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		...
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AMlgGameSession::OnDestroySessionComplete);
...
EOnlineAsyncTaskState::Type AMlgGameSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void AMlgGameSession::OnDestroySessionComplete(FName InSessionName, bool bWasSuccessful)
{
...
bool AMlgGameSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName InSessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
...
void AMlgGameSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AMlgGameSession::ChooseBestSession()
{
...
void AMlgGameSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AMlgGameSession::ContinueMatchmaking()
{
	ChooseBestSession();
...
void AMlgGameSession::OnNoMatchesAvailable()
{
...
//			SearchSettings = MakeShareable(new FOnlineSessionSearch(bIsLAN, bIsPresence));
...
bool AMlgGameSession::TravelToSession(int32 ControllerId, FName InSessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
